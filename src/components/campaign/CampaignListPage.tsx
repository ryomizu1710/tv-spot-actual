    ...
    <div className="mx-auto max-w-[1200px]">
      <div className="mb-6 flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-full bg-[#007AFF] px-5 py-2 text-[13px] font-medium text-white transition-all hover:bg-[#0062cc]"
        >
          <Plus size={15} />
          新規作成
        </button>
      ...
      {campaigns.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {campaigns.map((c) => (
            <div
              key={c.id}
              className="rounded-2xl bg-white/80 p-5 shadow-sm ring-1 ring-black/[0.04] backdrop-blur-xl transition-all duration-200 hover:shadow-md hover:ring-black/[0.08] cursor-pointer"
            ...
            >
              <div className="mb-3 flex items-start justify-between">
                ...
                <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${
                  ...
                  c.status === 'completed' ? 'bg-gray-100 text-[#86868b]' :
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {c.regions.map((r) => (
                  <span key={r} className="rounded-md bg-[#f5f5f7] px-2 py-0.5 text-[11px] font-medium text-[#86868b]">
                    {REGION_LABELS[r]}
                  </span>
                ))}
              </div>
